...
# tao ra sheet latest SW
        ...
        # data_validation_yes_no = '"YES, NO"'
        # for row in range(2, 8):
        #     data_validation_YN = DataValidation(type='list', formula1 = data_validation_yes_no)
        #     ws5.add_data_validation(data_validation_YN)
        #     data_validation_YN.add(ws5['F'+str(row)])
...
# tao ra sheet DID_DPT
        ...
        data_validation_data_FBL = '"APPL, FCN , APPL & FCN"'
...
# tao ra sheet general information
        ...
        ws5['A6'] = 'DID Check active session'
        cell_header = ws5.cell(5, 1)
        cell_header.fill = PatternFill(start_color='000066CC', end_color='000066CC', fill_type="solid")
        cell_header.border = border
        cell_header.font = font_text
...
def DID_baseSW(ws2, wb, id, number1, number2, number3, number4, direct):
    if direct == '':
        wb = load_workbook('BSWAvalue.xlsx')
    else:
...
def DID_latestSW(ws3, wb3, id, number1, number2, number3, number4, direct,dummy):
    if direct == '':
        wb3 = load_workbook('BSWAvalue.xlsx')
    else:
...
def DID_In_DPT(ws2, wb, id, number1, number2, number3, number4, direct):
    if direct == '':
        wb12 = load_workbook('BSWAvalue.xlsx')
    else:
        wb12 = load_workbook(str(direct))
    ws12 = wb12.active
    ws12 = wb12['DID_DPT']
    sheet3 = wb12.worksheets[2]
    row_count3 = sheet3.max_row
    for row in range(1, 2):
        for col in range(7, 8):
            char = get_column_letter(col)
            latestSW = ws12[char + str(row)].value
    ...
    while k < row_count3:
        for row in range(o, j):
            for col in range(1, 2):
                char = get_column_letter(col)
                row_list_DID_DID_DPT = ws12[char + str(row)].value
                row_list_DID_DID_DPT_lowercase = str(
                    row_list_DID_DID_DPT).lower()

        for row in range(o, j):
            for col in range(2, 3):
                char = get_column_letter(col)
                row_list_name_DID_DPT = ws12[char + str(row)].value
        ...
        for row in range(o, j):
            for col in range(3, 4):
                char = get_column_letter(col)
                row_list_length_byte_DID_DPT = ws12[char + str(row)].value

        for row in range(o, j):
            for col in range(4, 5):
                char = get_column_letter(col)
                row_list_ASCII_values_DID_DPT = ws12[char + str(row)].value
        
        for row in range(o, j):
            for col in range(5, 6):
                char = get_column_letter(col)
                row_list_hex_values_DID_DPT = ws12[char + str(row)].value
        ...
        if str(row_list_ASCII_values_DID_DPT) == 'None'  and str(row_list_hex_values_DID_DPT) == 'None':
            # hexvalue_DID_DPT = str(".{" + str(row_list_length_byte_DID_DPT) + "}")
        ...
        if c == 1:
            ws2.append(['ID_'+str(id),  '1.' + str(number1) + '.' + str(number2) + '.' + str(number3) + '.' + str(number4) + ' ' + str(row_list_DID_DID_DPT) + ' ' + str(row_list_name_DID_DPT), 'To check value of the DID ' + str(row_list_DID_DID_DPT), '1) Send service 0x22 to the camera for the DID ' +
                        str(row_list_DID_DID_DPT) + ' using physical addressing', '1) -', '1) RequestResponse(' + '22' + str(row_list_DID_DID_DPT_lowercase) + ','+'62'+str(row_list_DID_DID_DPT_lowercase) + str(hexvalue_DID_DPT) + ', Regexp)', 'Automated Testcase', 'implemented', latestSW, ''])
        if c == 0:
            ws2.append(['ID_'+str(id),  '1.' + str(number1) + '.' + str(number2) + '.' + str(number3) + '.' + str(number4) + ' ' + str(row_list_DID_DID_DPT) + ' ' + str(row_list_name_DID_DPT), 'To check value of the DID ' + str(row_list_DID_DID_DPT), '1) Send service 0x22 to the camera for the DID ' +
                        str(row_list_DID_DID_DPT) + ' using physical addressing', '1) -', '1) RequestResponse(' + '22' + str(row_list_DID_DID_DPT_lowercase) + ','+'62'+str(row_list_DID_DID_DPT_lowercase) + str(hexvalue_DID_DPT) + ', Equal)', 'Automated Testcase', 'implemented', latestSW, ''])
...
def TC_FBL():
    ...
    if direct == '':
        ...
        wb3 = load_workbook('RFvalue.xlsx')
        wb4 = load_workbook('RFvalue.xlsx')
        wb9 = load_workbook('RFvalue.xlsx')
        wb8 = load_workbook('TC_FBL.xlsx')
    else:
        wb = load_workbook(direct)
        wb3 = load_workbook(direct)
        wb4 = load_workbook(direct)
        wb9 = load_workbook(direct)
        wb8 = load_workbook(locate_save + '/'+'TC_FBL.xlsx')
    # wb3 = load_workbook(direct)
    ws = wb.active
    ws3 = wb3.active
    ws4 = wb4.active
    ...
    ws3 = wb3['RFvalue_latestSW']
    ws4 = wb4['General Information']
    ws9 = wb9['BSWAvalue.xlsx']
    sheet3 = wb9.worksheets[2]  
    sheet2 = wb3.worksheets[1]
    sheet = wb.worksheets[0]
    row_count = sheet.max_row
    row_count2 = sheet2.max_row
    row_count3 = sheet3.max_row
    tasks = row_count2 + row_count + 160 + row_count3

    ws8 = wb8.active
    ws8 = wb8['TC_FBL']
    sheet4 = wb8.worksheets[0]
    row_count4 = sheet4.max_row
    ...
    for row in range(2, 3):
        for col in range(2, 3):
            char = get_column_letter(col)
            latestSW = ws4[char + str(row)].value
    ...
    if locate_save == '':
        # print('Ok')
        wb2.save('TC_FBL.xlsx')
    else:
        wb2.save(locate_save + '/'+'TC_FBL.xlsx')

    if locate_save == '':
        wb7 = load_workbook('TC_FBL.xlsx')
    else:
        wb7 = load_workbook(locate_save + '/'+'TC_FBL.xlsx')
    ...
    if locate_save == '':
        wb7.save('TC_FBL.xlsx')
    else:
        wb7.save(locate_save + '/'+'TC_FBL.xlsx')
...
def run_program():
    ...
    if selected_test_option.get() == 'SRcheck':
        tkinter.messagebox.showinfo("GREAT!", "SRcheck")
...
Input_path_text = tk.StringVar()
Input_path_label = tk.Label(frame2, text='Select option test', font=('bold', 14), bg="#20bebe", fg="black").grid(row=6, column=0, sticky='w')
