...
def DID_baseSW(ws2, wb, id, number1, number2, number3, number4):
    ...
    while k < row_count:
        ...
        if str(row_list_values) == 'None':
            hexvalue = str(".{" + row_list_length_byte + "}")
            # print('dung')
        else:
...
def DID_latestSW(ws3, wb, id, number1, number2, number3, number4):
    ...
    while k < row_count2:
        ...
        for row in range(o, j):
            for col in range(2, 3):
                char = get_column_letter(col)
                row_list_name = ws3[ch ar + str(row)].value
                # print(ws3[char + str(row)].value)
        for row in range(o, j):
            for col in range(3, 4):
                char = get_column_letter(col)
...
def run_program():
    ...
    percent.set(str((id//tasks)*100)+"%")
    app.update_idletasks()
    ws2.append(['ID_'+str(id),  '1.' + str(number1) + '.' + str(number2) + '.' + str(number3) + '.' + str(number4) + ' UART flash'+baseSW, 'Detail information is mentioned in the ticket: '+ticket_baseSW,
                'Screen shot the successful flash proccess', 'Screen shot the successful flash proccess', 'Manual Testcase', 'Manual Testcase', 'implemented', baseSW, ''])
    id += 1
    bar['value'] += 1
    percent.set(str((id//tasks)*100)+"%")
    app.update_idletasks()
