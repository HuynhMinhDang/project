...
def DID_baseSW(ws2, wb, id, number1, number2, number3, number4):
    ...
    while k < row_count:
        ...
        hexvalue = ""
        length_byte = 0
        if str(row_list_values) == 'None':
            hexvalue = str(".{" + str(row_list_length_byte) + "}")
            # print('dung')
        else:
            ...
            # print(hexvalue)
            count_hexvalue = len(hexvalue)
            count_hexvalue = int(count_hexvalue) // 2
            # print(count_hexvalue)
            # print(type(row_list_length_byte))
            if str(count_hexvalue) < row_list_length_byte:
                length_byte = (int(row_list_length_byte) - count_hexvalue) ** 2
                hexvalue = str(hexvalue + ".{" + str(length_byte) + "}")
            # print(hexvalue)
        ...
        ws2.append(['ID_'+str(id),  '1.' + str(number1) + '.' + str(number2) + '.' + str(number3) + '.' + str(number4) + ' ' + str(row_list_DID) + ' ' + str(row_list_name), 'To check value of the DID ' + str(row_list_DID), '1) Send service 0x22 to the camera for the DID ' +
                    str(row_list_DID) + ' using physical addressing', '1) -', '1) RequestResponse(' + '22' + str(row_list_DID) + ','+'62'+str(row_list_DID)+str(hexvalue) + ', Equal)', 'Automated Testcase', 'implemented', baseSW, ''])
        number4 += 1
        o += 1
        j += 1
        k += 1
...
def DID_latestSW(ws3, wb, id, number1, number2, number3, number4):
    ...
    while k < row_count2:
        ...
        for row in range(o, j):
            for col in range(3, 4):
                char = get_column_letter(col)
                row_list_length_byte = ws3[char + str(row)].value
                # print(ws[char + str(row)].value)
                # print(row_list_length_byte)
        for row in range(o, j):
            for col in range(4, 5):
        ...
        ws2.append(['ID_'+str(id),  '1.' + str(number1) + '.' + str(number2) + '.' + str(number3) + '.' + str(number4) + ' ' + str(row_list_DID) + ' ' + str(row_list_name), 'To check value of the DID ' + str(row_list_DID), '1) Send service 0x22 to the camera for the DID ' +
                    str(row_list_DID) + ' using physical addressing', '1) -', '1) RequestResponse(' + '22' + str(row_list_DID) + ','+'62'+str(row_list_DID)+str(hexvalue) + ', Equal)', 'Automated Testcase', 'implemented', latestSW, ''])
        number4 += 1
        o += 1
        j += 1
        k += 1
...
def variant_base_sw(id, number1, number2, number3, number4, tasks):
    ...
    ws2.append(['ID_'+str(id),  '1.' + str(number1) + '.' + str(number2) + '.' + str(number3) + '.' + str(number4) + ' Select_and_check_variant', 'To Select and check variant', '1) Tester Present is ON\n2) Change to Extended session with Service 0x10 03\n3) Security unlock ON\n4) wait\n5) Security unlock OFF\n6) Select variant\n7) Check variant', '1) -\n2) -\n3) -\n4) -\n5) -\n6) -\n7) -',
                '1) envvar(EnvTesterPresentOnOff(1;0))\n2) RequestResponse(1003, 5003.*, Regexp)\n3) envvar(EnvLogInLevel1_1(0;0))\n4) wait(1000)\n5) envvar(EnvLogInLevel1_1(0;0))\n6) RequestResponse(2e'+ str(row_Variant_BaseSW) +', 6e' + str(row_Variant_BaseSW) + ', Equal)\n7) RequestResponse(22'+ str(row_DID_check_variant_BaseSW) +', 62'+ str(row_Variant_BaseSW) +', Equal)', 'Automated Testcase', 'implemented', baseSW, ''])
    return id
...
def variant_latest_sw(id, number1, number2, number3, number4, tasks):
    ...
    ws2.append(['ID_'+str(id),  '1.' + str(number1) + '.' + str(number2) + '.' + str(number3) + '.' + str(number4) + ' Select_and_check_variant', 'To Select and check variant', '1) Tester Present is ON\n2) Change to Extended session with Service 0x10 03\n3) Security unlock ON\n4) wait\n5) Security unlock OFF\n6) Select variant\n7) Check variant', '1) -\n2) -\n3) -\n4) -\n5) -\n6) -\n7) -',
                '1) envvar(EnvTesterPresentOnOff(1;0))\n2) RequestResponse(1003, 5003.*, Regexp)\n3) envvar(EnvLogInLevel1_1(0;0))\n4) wait(1000)\n5) envvar(EnvLogInLevel1_1(0;0))\n6) RequestResponse(2e'+str(row_Variant_LatestSW) + ', 6e'+str(row_Variant_LatestSW) +', Equal)\n7) RequestResponse(22' + str(row_DID_check_variant_LatestSW) + ', 62' + str(row_Variant_LatestSW) + ', Equal)', 'Automated Testcase', 'implemented', latestSW, ''])
    return id
# -------------------------------------------------------------------------------------------------------------
...
instruction = tk.Label(
    app, text="Welcome to Rflash tool create by dev HUYNH MINH DANG", font=("helvetica", 14))
